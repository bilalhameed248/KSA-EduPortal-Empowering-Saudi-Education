...
<div class="row">
	<div class="col-sm-12">
		<div class="card-box">
			...
			<form method="POST" action="{{route('get_report_l')}}" enctype="multipart/form-data">
			@csrf
				...
				<div class="col-lg-6 p-t-20">
					<div
						...
						<select required id="allgrade" class="form-control">
							<option selected disbaled>Select Grade</option>
							@foreach($allbranch as $allbranch1)
					        <option value="{{$allbranch1->grade_id}}">{{$allbranch1->grade_name}}</option>
					        @endforeach
					    </select>
...
<script type="text/javascript">
    ...
    $('#allsubjects').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_student_l') }}',
            data: {
                _token: "{{ csrf_token() }}",
                subject_id: subject_id, class_id: class_id, grade_id: grade_id
            },
            success: function(response) 
            {
            	var len = response.length;
