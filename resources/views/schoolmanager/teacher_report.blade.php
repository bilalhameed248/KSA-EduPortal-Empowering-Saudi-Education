...
<div class="row">
	<div class="col-sm-12">
		<div class="card-box">
			...
			<form method="POST" action="{{route('get_report_school')}}" enctype="multipart/form-data">
			@csrf
...
<script type="text/javascript">
	$('#allbranch').on('change', function() {
		...
		$.ajax({
            ...
            success: function(response) 
            {
            	...
            	$("#allschool").empty();
            	var output='<option selected disbaled>Select School</option><option value="-322">All</option>';
                for (var i = 0; i < len; i++) 
                {
    ...
    $('#allblock').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_grade_school') }}',
            data: {
                _token: "{{ csrf_token() }}",
                block_id: block_id, branch_id:branch_id, school_id:school_id
            },
            success: function(response) 
            {
            	var len = response.length;
    ...
    $('#allgrade').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_class_school') }}',
            data: {
                _token: "{{ csrf_token() }}",
                grade_id: grade_id, block_id: block_id, branch_id:branch_id, school_id:school_id
            },
            success: function(response) 
            {
            	// alert(response);
    ...
    $('#allclass').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_subjects_school') }}',
            data: {
                _token: "{{ csrf_token() }}",
                class_id: class_id, grade_id: grade_id, block_id: block_id, branch_id:branch_id, school_id:school_id
            },
            success: function(response) 
            {
            	// alert(response);
    ...
    $('#allsubjects').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_student_school') }}',
            data: {
                _token: "{{ csrf_token() }}",
                subject_id: subject_id, class_id: class_id, grade_id: grade_id, block_id: block_id, branch_id:branch_id, school_id:school_id
            },
            success: function(response) 
            {
            	var len = response.length;
