...
<div class="row">
	<div class="col-sm-12">
		<div class="card-box">
			...
			<form method="POST" action="{{route('get_report_hm')}}" enctype="multipart/form-data">
			@csrf
			<div class="card-body row">
				...
				<div class="col-lg-6 p-t-20">
					<div
						...
						<select required id="allblock" class="form-control">
							<option selected disbaled>Select Block</option>
							@foreach($allbranch as $allbranch1)
					        <option value="{{$allbranch1->block_id}}">{{$allbranch1->block_name}}</option>
					        @endforeach
					    </select>
				...
				<div class="col-lg-12 p-t-20 text-center">
					...
					<button type="button"
						class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect m-b-10 btn-default" onclick="location.href='{{ url('hm_report') }}'">Cancel</button>
				</div>
			</div>
		</form>
		</div>
...
<script type="text/javascript">
    ...
    $('#allgrade').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_class_hm') }}',
            data: {
                _token: "{{ csrf_token() }}",
                grade_id: grade_id, block_id: block_id
            },
            success: function(response) 
            {
            	// alert(response);
    ...
    $('#allclass').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_subjects_hm') }}',
            data: {
                _token: "{{ csrf_token() }}",
                class_id: class_id, grade_id: grade_id, block_id: block_id
            },
            success: function(response) 
            {
            	// alert(response);
    ...
    $('#allsubjects').on('change', function() {
		...
		$.ajax({
            type: 'POST',
            url: '{{ route('get_student_hm') }}',
            data: {
                _token: "{{ csrf_token() }}",
                subject_id: subject_id, class_id: class_id, grade_id: grade_id, block_id: block_id
            },
            success: function(response) 
            {
            	var len = response.length;
